...
public class MeasureDetailController {
    ...
    public LjBaseResponse<NullMsgVo> exportExcel(HttpServletRequest request, @Valid MeasureDetailExportExcelReq req){
        ...
        try {
            ...
            ProjectBase curProj = (ProjectBase)sessionInfo.getBaseInfo("cur_proj");
            measureDetailService.exportExcelAsync(user.getUserId(), curProj.getId(), req.getList_id());
        } catch (Exception e) {
            log.error("error:" + e);
